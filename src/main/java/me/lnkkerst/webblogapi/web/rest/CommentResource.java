...
import me.lnkkerst.webblogapi.service.CommentQueryService;
import me.lnkkerst.webblogapi.service.CommentService;
...
/** REST controller for managing {@link me.lnkkerst.webblogapi.domain.Comment}. */
@RestController
...
public class CommentResource {
   ...
   *     comment, or with status {@code 400 (Bad Request)} if the comment has already an ID.
  ...
  public ResponseEntity<Comment> createComment(@Valid @RequestBody Comment comment)
    ...
    if (comment.getId() != null) {
      throw new BadRequestAlertException(
          "A new comment cannot already have an ID", ENTITY_NAME, "idexists");
    }
   ...
   *     comment, or with status {@code 400 (Bad Request)} if the comment is not valid, or with
   *     status {@code 500 (Internal Server Error)} if the comment couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/{id}")
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, false, ENTITY_NAME, comment.getId().toString()))
        .body(result);
  }

  /**
   * {@code PATCH /comments/:id} : Partial updates given fields of an existing comment, field will
   ...
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the updated
   *     comment, or with status {@code 400 (Bad Request)} if the comment is not valid, or with
   *     status {@code 404 (Not Found)} if the comment is not found, or with status {@code 500
   *     (Internal Server Error)} if the comment couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PatchMapping(
      value = "/{id}",
   ...
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the list of comments in
   *     body.
   */
  @GetMapping("")
  public ResponseEntity<List<Comment>> getAllComments(
      CommentCriteria criteria, @org.springdoc.core.annotations.ParameterObject Pageable pageable) {
   ...
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the comment, or
