...
import me.lnkkerst.webblogapi.service.PostQueryService;
import me.lnkkerst.webblogapi.service.PostService;
...
/** REST controller for managing {@link me.lnkkerst.webblogapi.domain.Post}. */
@RestController
...
public class PostResource {
  ...
  public PostResource(
      PostService postService, PostRepository postRepository, PostQueryService postQueryService) {
   ...
   *     post, or with status {@code 400 (Bad Request)} if the post has already an ID.
  ...
  public ResponseEntity<Post> createPost(@Valid @RequestBody Post post) throws URISyntaxException {
    ...
    if (post.getId() != null) {
      throw new BadRequestAlertException(
          "A new post cannot already have an ID", ENTITY_NAME, "idexists");
    }
   ...
   *     or with status {@code 400 (Bad Request)} if the post is not valid, or with status {@code
   *     500 (Internal Server Error)} if the post couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/{id}")
  public ResponseEntity<Post> updatePost(
      @PathVariable(value = "id", required = false) final Long id, @Valid @RequestBody Post post)
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, false, ENTITY_NAME, post.getId().toString()))
        .body(result);
  }

  /**
   * {@code PATCH /posts/:id} : Partial updates given fields of an existing post, field will ignore
   * if it is null
   *
   * @param id the id of the post to save.
   * @param post the post to update.
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the updated post,
   *     or with status {@code 400 (Bad Request)} if the post is not valid, or with status {@code
   *     404 (Not Found)} if the post is not found, or with status {@code 500 (Internal Server
   *     Error)} if the post couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PatchMapping(
      value = "/{id}",
  ...
  public ResponseEntity<Post> partialUpdatePost(
      @PathVariable(value = "id", required = false) final Long id, @NotNull @RequestBody Post post)
      throws URISyntaxException {
  ...
  public ResponseEntity<List<Post>> getAllPosts(
      PostCriteria criteria, @org.springdoc.core.annotations.ParameterObject Pageable pageable) {
   ...
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the post, or with
